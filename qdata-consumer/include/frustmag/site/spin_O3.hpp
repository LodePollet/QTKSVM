...
struct spin_O3 : Eigen::Vector3d {
    ...
    static spin_O3 random(RNG & rng) { //, double cos_theta_0 = -1) { //end of line used for classical spins

        /* TETRA POVM Spin 1/2 Decker orientation, see ref [Decker03]
        int rndint = std::uniform_int_distribution<> {0, 3}(rng);
        spin_O3 ret;
        if (rndint == 0)      //Pt1
            ret <<  std::sqrt(2./3.), 0,  1./std::sqrt(3.);
        else if (rndint == 1) //Pt2
            ret << -std::sqrt(2./3.), 0,  1./std::sqrt(3.);
        else if (rndint == 2) //Pt3
            ret <<  0,  std::sqrt(2./3.), -1./std::sqrt(3.);
        else if (rndint == 3) //Pt4
            ret <<  0, -std::sqrt(2./3.), -1./std::sqrt(3.);
        else
            std::cout << "site_type::random() bad value encountered " << rndint << std::endl;
        return ret;
        */

        ///* Pauli-6 POVM Spin 1/2
        int rndint = std::uniform_int_distribution<> {0, 5}(rng);
        spin_O3 ret;
        if (rndint == 0)      //xup
            ret <<  +1,  0,  0;
        else if (rndint == 1) //xdn
            ret <<  -1,  0,  0;
        else if (rndint == 2) //yup
            ret <<   0, +1,  0;
        else if (rndint == 3) //ydn
            ret <<   0, -1,  0;
        else if (rndint == 4) //zup
            ret <<   0,  0, +1;
        else if (rndint == 5) //zdn
            ret <<   0,  0, -1;
        else
            std::cout << "site_type::random() bad value encountered " << rndint << std::endl;
        return ret;
