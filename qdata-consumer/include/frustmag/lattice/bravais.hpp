...
struct bravais {
    static constexpr size_t n_basis = N_BASIS;
    ...
    unitcell_iterator cellsbegin() {
        return {cells_.begin(), 0, plengths.data(), periodic};
    }

    unitcell_const_iterator cellsbegin() const {
        return {cells_.begin(), 0, plengths.data(), periodic};
    }

    unitcell_iterator cellsend() {
        return {cells_.end(), static_cast<difference_type>(plengths.back()),
                    plengths.data(), periodic};
    }

    unitcell_const_iterator cellsend() const {
        return {cells_.end(), static_cast<difference_type>(plengths.back()),
                    plengths.data(), periodic};
    }
    ...
    lengths_type const& get_plengths() const {
        return plengths;
    }
