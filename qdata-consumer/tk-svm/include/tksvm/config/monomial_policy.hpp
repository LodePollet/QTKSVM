    ...
    , private SymmetryPolicy
{
    ...
    virtual size_t size () const override final {
        ...
        size_t num_chan = ElementPolicy::range()/ElementPolicy::n_block();
        size_t num_spins = ElementPolicy::n_block();
        auto const binom = [](int n, int k) {
            auto binom_impl = [](int n, int k, auto& binom_ref) mutable {
                if (k == 0 || k == n) return 1;
                return binom_ref(n - 1, k - 1, binom_ref) + binom_ref(n - 1, k, binom_ref);
            };
            return binom_impl(n, k, binom_impl);
        };
        return binom(num_spins, rank_) * std::pow(num_chan, rank_);
    }

    virtual bool block_more_than_once(indices_t const& ind) const final {
        for (auto it = ind.cbegin(); it != ind.cend(); ++it) {
            for (auto it2 = it+1; it2 != ind.cend(); ++it2) {
                if (ElementPolicy::block(*it) == ElementPolicy::block(*it2)) return true;
            }
        }
        return false;
    }
    ...
    virtual matrix_t rearrange (introspec_t const& coeff,
                                indices_t const& bi) const override final {  
        symmetry_policy::none no_symm;
        size_t no_symm_size_Y = no_symm.size(ElementPolicy::range()
                                           / ElementPolicy::n_block(), rank_);
        size_t no_symm_size_X = no_symm.size(ElementPolicy::range(), rank_);
        matrix_t out(boost::extents[no_symm_size_X][no_symm_size_Y]);  
/*
        indices_t i_ind(rank_);
        std::vector<size_t> i_ind_lookup(no_symm_size_Y);
        for (size_t i = 0; i < size(); ++i, advance_ind(i_ind)) {
            do {
                indices_t ind_block = block_indices(i_ind);
                size_t out = rearranged_index(component_indices(i_ind));
                if (std::equal(bi.begin(), bi.end(), ind_block.begin()))
                    i_ind_lookup[out] = i;
            } while (unsymmetrize && transform_ind(i_ind));
        }

        for (size_t i_out = 0; i_out < no_symm_size_Y; ++i_out) {
            size_t i = i_ind_lookup[i_out];
            indices_t j_ind(rank_);
            for (size_t j = 0; j < size(); ++j, advance_ind(j_ind)) {
                do {
                    size_t j_out = rearranged_index(j_ind);
                    out[j_out][i_out] = coeff.tensor({j, i}) / (weights_[i] * weights_[j]);
                } while (unsymmetrize && transform_ind(j_ind));
            }
        }
*/
