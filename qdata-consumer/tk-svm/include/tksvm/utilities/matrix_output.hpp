...
inline double normalize_vector (boost::multi_array<double,1> & vec) {
    double absmax = 0.;
    for (size_t i = 0; i < vec.size(); ++i)
        if (std::abs(absmax) < std::abs(vec[i]))
            absmax = vec[i];
    for (size_t i = 0; i < vec.size(); ++i)
            vec[i] /= absmax;
    return absmax;
                          ...
                          Palette const& pal)
{
    /* PPM output */ {
        ...
        //proper flattening for stripe mode
        if (mat.shape()[0] != mat.shape()[1]){
            for (size_t i = 0; i < mat.shape()[0]; i++)
                for (size_t j = 0; j < mat.shape()[1]; j++)
                    flat[0][(mat.shape()[1]-j-1) * mat.shape()[0] + i] = mat[i][j];
        }
...
inline void write_vector (boost::multi_array<double,1> const& vec,
                          std::string basename)
{
    /* TXT output */ {
        std::ofstream txt(basename + ".txt");
