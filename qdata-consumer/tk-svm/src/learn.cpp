...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        // In ALPSCore v2.2.0 alps::params() throws error if hdf5 archive is not the only argument
        // Workaround: copy argv and leave out last argument (typically "-i")
        char** argv_cp = (char**) malloc((argc) * sizeof *argv_cp);
        for (int i = 0; i < argc-1; i++) {
            argv_cp[i] = argv[i];
        }
        argv_cp[argc-1] = NULL;
        alps::params parameters(argc-1, argv_cp);
        free(argv_cp);
        ...
        if (!cmdl["--statistics-only"]) {
            ...
            // set cache_size of svm
            if(cmdl("--cache_size"))
                    cmdl("cache_size") >> kernel_params.cache_size();

            std::cout << "Creating SVM model..."
                      << " (nu = " << parameters["nu"].as<double>()
                      << ", cache_size: " << kernel_params.cache_size()
