...
In situations where many separate `*.clone.h5` files ought to be merged,
specifying them individually on the command line can become cumbersome or even
impossible (as the `--merge` token is limited to a total buffer size of 1000
characters as imposed by ALPSCore). As an alternative, one can provide a text
file holding the names of additional `*.clone.h5`. Such a file is distinguished
by enclosing its path in square brackets; _e.g._:

```bash
$ gauge-learn first.clone.h5 --merge=second.clone.h5:third.clone.h5:[list.txt]
```

where each line `list.txt` is parsed according to the same rules as the original
`--merge` argument, _e.g._

```
four.clone.h5
five.clone.h5:six.clone.h5
seven.clone.h5
[another_list.txt]
```
...
```bash
$ gauge-coeffs Td-hyperplane.out.h5 -u
$ gauge-coeffs Td-hyperplane.out.h5 -u --block=[0,1,2:0,1,2]
$ gauge-coeffs Td-hyperplane.out.h5 -u --block=[0,1,2:0,1,2] --remove-self-contractions
```
Note: In older versions of tksvm the block was specified using color indices `[lmn:lmn]`.
However, in case of large cluster size one might run out of letters quickly,
which is why the color indices where replaced by comma separated numbers ranging from 0-9999.
...
The program has three major modes: extraction of the full coefficient matrix,
*single-block mode* and *stripe mode* where a single block or a stripe of blocks is targeted and extracted exclusively.
This is done by specifying the command line parameter `--block=<block-spec>` or `--stripe=<stripe-spec>`
where `<block-spec>`/`<stripe-spec>` identifies a block/stripe by its indices. _E.g._ in the case
of the tetrahedral (rank-3) order, the non-trival block can be found by
specifying `--block=[0,1,2:0,1,2]`. 
The *stripe mode* is particularly usefull to reduce complexity. At larger cluster size
extracting the full coefficient matrix can become a bottle neck, especially at higher ranks. 
*stripe mode* reduces the complexity from quadratic with number of blocks to linear.
To use it one has to specify a block-row index of the coefficient matrix, e.g. `--stripe=[0,1,2]`.
...
| `cache_size`   | `100`   | Size of the kernel cache, in unit of MB                                            |
